...
export class PhonicsLab{
  private group:Group;private page=0;private selected=0;private listen=false;private muted=false;private slow=false;private done=new Set<string>();
  ...
  private action(a:string){
    ...
    if(a==='read'){this.done.add(this.cards()[this.selected].id);this.note='✓ 我念過了';this.render();return;}
  ...
  private render(){
    ...
    const content=this.listen?(!this.gate?`<div class="ph-finish"><strong>✓ ${this.correct} / ${this.queue.length}</strong><p>練習完成</p><button data-action="restart">再玩一次</button></div>`:`<div class="ph-listen-prompt"><button class="ph-big-sound" data-action="hear" aria-label="播放題目發音">${sound}</button><p>聽到哪個單字？</p><small>${this.question+1} / ${this.queue.length}</small></div><div class="ph-answer-grid">${this.options.map(c=>`<div class="ph-answer ${this.gate!.selected===c.id?'is-selected':''} ${this.gate!.checked&&c.id===q.id?'is-correct':''}"><button class="ph-pick" data-select="${c.id}" aria-pressed="${this.gate!.selected===c.id}" ${this.gate!.checked?'disabled':''}><img src="${c.image}" alt=""><strong>${c.word}</strong>${this.gate!.selected===c.id?'<i>✓</i>':''}</button><button class="ph-preview" data-preview="${c.id}" aria-label="試聽 ${c.word}">${sound}</button></div>`).join('')}</div>`):`<div class="ph-card-grid">${cards.slice(this.page*6,this.page*6+6).map((c,i)=>`<button data-card="${this.page*6+i}" aria-pressed="${this.selected===this.page*6+i}" aria-label="${this.group==='reading'?c.word:c.ipa+'，'+c.word}">${this.group==='reading'?`<img src="${c.image}" alt="">`:`<strong class="ph-ipa">/${c.ipa}/</strong>`}<span>${c.word}</span>${this.done.has(c.id)?'<i>✓</i>':''}</button>`).join('')}</div><article class="ph-detail"><img src="${card.image}" alt="${card.word==='measure'?'用尺量長度的例字提示':card.word}"><div><strong class="ph-word">${card.word}</strong><span class="ph-symbol">/${card.ipa}/ · ${card.spelling}</span></div><p>${escape(this.group==='reading'?card.sentence:card.hint)}</p><div class="ph-detail-actions"><button data-action="hear" aria-label="聽例字 ${card.word}">${sound}</button><button data-action="sentence" aria-label="聽短句">${sound}<span>短句</span></button><button data-action="read" aria-label="我念過了">✓</button></div></article>`;
    this.host.innerHTML=`<section class="ph-shell" aria-label="一年級發音工坊"><header><button data-action="back" aria-label="返回英文">←</button><h2>發音工坊</h2><span>美式例字</span><button data-action="slow" aria-pressed="${this.slow}" aria-label="慢速發音">0.6×</button><button data-action="mute" aria-pressed="${this.muted}" aria-label="${this.muted?'開啟':'關閉'}語音">${this.muted?'◌':sound}</button></header><nav aria-label="發音單元">${Object.entries(names).map(([id,n])=>`<button data-action="${id}" aria-pressed="${this.group===id&&!this.listen}">${n}</button>`).join('')}<button data-action="listen" aria-pressed="${this.listen}">聽音四選一</button></nav><div class="ph-body ${this.listen?'is-listening':''}">${content}</div><footer><p role="status">${this.note||'點卡聽例字，跟著念'}</p>${this.listen?(this.gate?`<button class="ph-check" data-action="${this.gate.checked?'continue':'check'}" ${!this.gate.selected?'disabled':''}>${this.gate.checked?'下一題 →':'檢查 ✓'}</button>`:''):`<button data-action="prev" aria-label="上一頁">‹</button><span>${this.page+1} / ${Math.ceil(cards.length/6)}</span><button data-action="next" aria-label="下一頁">›</button>`}</footer></section>`;
  }
