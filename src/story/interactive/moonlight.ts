...
export const moonlightObjects:Record<string,ObjectSpec>={
 moonflower:{id:'moonflower',word:'flower',image:'foreground.png',x:26,y:47,w:15,h:11},
 gail:{id:'gail',word:'Gail',kind:'girl',image:gailPoseImage('standing'),x:38,y:66,w:22,h:53},
 light:spot('light','light',73,25,18,22),moth:{...spot('moth','moth',68,30,16,20),image:'moth.png'},window:spot('window','window',77,30,28,45),garden:spot('garden','garden',50,45,38,42),flower:spot('flower','flower',74,59,26,35),bell:{...spot('bell','bell',70,22,10,24),image:'bell.png'},path:spot('path','path',57,72,30,25),bridge:spot('bridge','bridge',67,47,30,27),fog:spot('fog','fog',53,63,42,29),frog:{...spot('frog','frog',65,73,14,21),image:'frog.png'},lily:spot('lily','lily pad',67,68,27,22),mushroom:spot('mushroom','mushroom',22,36,28,48),moon:spot('moon','moon',78,13,16,20),firefly:spot('firefly','firefly',70,25,27,27),tree:spot('tree','moon tree',63,33,44,54),together:spot('together','together',50,54,48,56),bed:spot('bed','bed',48,67,58,42)
};
...
export const moonlightScenes:Record<string,SceneSpec>=Object.fromEntries(names.map((id,i)=>[id,{name:moonlightTranslations[i],caption:captions[i],alt:`《瑜瑜與月光花園》第 ${i+1} 幕手繪插畫`,sheet:`backgrounds-${String(Math.floor(i/4)*4+1).padStart(2,'0')}-${String(Math.floor(i/4)*4+4).padStart(2,'0')}.webp`,cell:i%4}])) as Record<string,SceneSpec>;
...
export function moonlightPages(level:ReadingLevel):StoryPage[]{return moonlightText.map((versions,i)=>{
 const answer=targets[i];const objects=['gail',answer,...(i===19?['moonflower']:[]),...(answer!=='moth'&&i<19?['moth']:[]),...(answer!=='frog'&&i>=8&&i<=18?['frog']:[]),...(answer!=='bell'&&i>=14&&i<=18?['bell']:[]),...(i===15?['together']:[])];
 ...
 return {title:titles[i],scene:names[i],lines:[...versions[level]],translation:moonlightTranslations[i],words:vocabulary[i],objects:[...new Set(objects)],placements:{...placements[i],gail:{x:38,y:66,w:22,h:53,...([0,1,4,8,11,12,19].includes(i)?{image:gailPoseImage('seated'),y:73,w:28,h:35}:{}),...(i===0?{x:35,y:62}:{}),...(i===19?{x:40,y:50}:{}),...(i===9?{x:29,y:66}:{}),...(i>=14&&i<=17?{x:44,y:67}: {})},...(i>=14?{bell:{x:68,y:25,w:10,h:24},moth:{x:54,y:32,w:13,h:17}}:{})},mission,checkpoint:moonlightCheckpoints.has(i),action:'dream'};
});}
export function moonlightChoreography(_level:ReadingLevel):Record<number,Record<number,StoryMotion[]>>{return {
 2:{0:[{actor:'gail',x:49,y:65,walk:true},{actor:'moth',x:76,y:25}]},
 6:{0:[{actor:'gail',x:51,y:64,walk:true},{actor:'moth',x:69,y:30}]},
 7:{0:[{actor:'gail',x:49,y:66,walk:true}]},
 9:{1:[{actor:'frog',x:72,y:67}]},
 10:{0:[{actor:'gail',x:51,y:65,walk:true},{actor:'frog',x:65,y:78}]},
 13:{0:[{actor:'gail',x:53,y:63,walk:true}]},
 16:{1:[{actor:'frog',x:57,y:40},{actor:'moth',x:68,y:26}]},
 17:{0:[{actor:'bell',x:68,y:25,rotate:15}]}
 };}
