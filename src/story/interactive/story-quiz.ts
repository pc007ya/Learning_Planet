...
export function quizArt(id:QuizArt){const frame=document.createElement('span');frame.className='quiz-art art-'+id;frame.setAttribute('role','img');frame.setAttribute('aria-label',artNames[id]);
 ...
 else if(id==='gail'){const img=document.createElement('img');img.src='images/story/little-star-v1/gail-paper-clean.png';img.className='paper-girl';img.alt='';inner.append(img);}
 ...
 return frame;}
export function mountStoryQuiz(host:HTMLElement,questions:StoryQuestion[],state:QuizState,options:{level:string;say:(text:string)=>void;back:()=>void;save:(score:number)=>void;ending?:{title:string;line:string;image:string;alt?:string}}){
 ...
 if(state.complete){const score=quizScore(state,questions);if(options.ending){const img=document.createElement('img');img.className='quiz-ending-image';img.src=options.ending.image;img.alt=options.ending.alt||'Gail 與 Ella 分享風車';card.append(img);}else card.append(quizArt('wing'));card.append(element('h1','',options.ending?.title||'Brave Little Dragon'),element('p','',`完成了！答對 ${score} / ${questions.length} 題`),element('p','',options.ending?.line||'Try once. Try twice. Spread your wings—and touch the sky!'));const actions=element('div','quiz-actions');actions.append(button('再試一次','重新測驗',()=>{Object.assign(state,freshQuiz());draw();}),button('回故事','回到故事',options.back));card.append(actions);return;}
