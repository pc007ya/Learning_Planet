...
export class Mini4wdLab{
  ...
  private action(id:string){
    ...
    if(id.startsWith('track-')&&!this.running){const k=id.slice(6) as TrackKind;if(k in tracks){this.stop();this.track=k;this.view?.setTrack(k);this.render();this.tell(tracks[k],k==='flat'?'平坦的跑道，先練習三圈。':k==='bridge'?'連續的坡橋，觀察上坡和下坡的速度。':'斜坡後面有缺口，觀察車子起跳和落地。');}return;}
