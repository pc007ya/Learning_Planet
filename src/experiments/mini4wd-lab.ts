...
import {DEFAULT_SETUP,type Setup,type CarState,type TrackKind} from './mini4wd-physics';
import {gearPicture,ballastPicture,questionPicture,trackProfilePicture} from './mini4wd-art';
...
icons.ghost='M5 20V9a7 7 0 0 1 14 0v11l-3-2-4 2-4-2-3 2M9 9v2M15 9v2';
...
type RecordRow={setup:Setup;track:TrackKind;time:number;max:number;contacts:number;landings:number;finished:boolean};
const tracks={flat:'平道',bridge:'坡橋',jump:'跳台'};
const lessons=[
...
export class Mini4wdLab{
  ...
  private track:TrackKind='flat';private trackPhotos:Record<string,string>={};private optionPhotos:Record<string,string>={};
  private root:HTMLElement;private view?:Mini4wdView;private voice=new SceneNarration();private abort=new AbortController();
  private page:Page='learn';private previous:Page='learn';private lesson=0;private partPage=0;private selected='';private installed=completeParts();private started=false;
  ...
  constructor(private host:HTMLElement){
    ...
    this.root.querySelector('.m4-hud')!.insertAdjacentHTML('beforeend','<div data-shadow hidden><b>—</b><small>影子 · 上次秒數</small></div>');
    ...
    try{this.view=new Mini4wdView(this.root.querySelector('.m4-stage')!);this.view.configure(this.setup);this.thumbnails=this.view.thumbnails();this.optionPhotos=this.view.optionThumbnails();this.trackPhotos=this.view.trackThumbnails();this.view.onPick=id=>this.select(id);this.view.onUpdate=c=>this.updateRace(c);}catch(e){console.error('Mini 4WD WebGL',e);this.tell('這個裝置未能開啟 3D。請啟用硬體加速，或換支援 WebGL 的瀏覽器。');}
  ...
  private render(){
    ...
    if(this.page==='learn'){
      const l=lessons[this.lesson];panel.innerHTML=`<div class="m4-lesson-cards">${lessons.map((item,i)=>`<button class="m4-object-card" data-m4="lesson-${i}" aria-label="${item.title}" aria-pressed="${this.lesson===i}"><img src="${this.thumbnails[item.part]||''}" alt=""><span>${['電池','齒輪','傳動','導輪'][i]}</span></button>`).join('')}</div><div class="m4-dock-actions">${button('power',this.powered?'關閉電源':'開啟電源','power')}${button('replay','聽解說')}${button('lesson-next',this.lesson===3?'去組裝':'下一步','next')}</div><p class="m4-sr">${l.voice}</p>`;
    ...
    else if(this.page==='race')panel.innerHTML=`<div class="m4-track-cards">${(Object.keys(tracks) as TrackKind[]).map(k=>`<button class="m4-object-card" data-m4="track-${k}" aria-label="${tracks[k]}賽道" aria-pressed="${this.track===k}" ${this.running?'disabled':''}><img src="${this.trackPhotos[k]||''}" alt="">${trackProfilePicture(k)}<span>${tracks[k]}</span></button>`).join('')}</div><div class="m4-race-options">${button('camera',this.first?'車頭視角':'追車視角','camera')}${button('sound',this.sound?'聲音開':'聲音關','voice')}<button data-m4="motion" aria-label="穩定鏡頭" aria-pressed="${this.reduced}">${icon('eye')}</button>${button('collision','雙車碰撞測試','parts')}<button data-m4="ghost" aria-label="挑戰影子車" title="同一跑道上一趟完成的自己" aria-pressed="${this.shadow}" ${this.running?'disabled':''}>${icon('ghost')}</button></div><div class="m4-dock-actions"><button class="m4-go" data-m4="run" aria-label="開始試跑"><img src="images/experiments/mini4wd-v2/race-flag.png" alt=""><span>出發</span></button>${button('stop','停止','stop')}</div>`;
      ...
      overlay.innerHTML=`<h3>比較試跑結果</h3>${entries.length?entries.map((r,i)=>`<article class="m4-record"><b>第 ${this.recordPage*2+i+1} 次 · ${r.finished?'完成三圈':'中途停止'}</b><p>${tracks[r.track]} · ${setupLabel(r.setup)}</p><div><strong>${r.time.toFixed(2)} <small>秒</small></strong><strong>${(r.max*.18).toFixed(1)} <small>km/h 最高</small></strong></div><p>護欄撞擊 ${r.contacts} 次 · 落地 ${r.landings} 次</p></article>`).join(''):'<p>還沒有紀錄。把車組好，去試跑吧！</p>'}`;
    }else{
      ...
      overlay.innerHTML=`<h3>${q.q}</h3>${questionPicture(this.quizIndex,this.thumbnails)}<div class="m4-answer-grid">${q.ids.map(id=>`<button data-answer="${id}" aria-label="${PARTS.find(p=>p.id===id)!.name}"><img src="${this.thumbnails[id]||''}" alt="${PARTS.find(p=>p.id===id)!.name}"></button>`).join('')}</div><p class="m4-feedback" role="status">點一張圖片</p>`;
      this.voice.last=q.q;
    }
  ...
  private renderTune(panel:Element){
    ...
    const group=groups[this.tunePage];
    panel.innerHTML='<nav class="m4-category-tabs" aria-label="改裝分類">'+groups.map((g,i)=>'<button data-m4="category-'+i+'" aria-label="'+g.name+'" aria-pressed="'+(i===this.tunePage)+'"><img src="'+(this.thumbnails[g.part]||'')+'" alt=""><span>'+g.name+'</span></button>').join('')+'</nav><div class="m4-tune-cards" role="group" aria-label="'+group.name+'">'+group.items.map(([v,label])=>{
      const [key,value]=v.split(':');const art=key==='gear'?gearPicture(Number(value)):key==='ballast'?ballastPicture(value==='rear'):key==='color'?'<i class="m4-color-orb" style="--paint:'+value+'"></i>':'<img src="'+(this.optionPhotos[v]||'')+'" alt="">';
      return '<button class="m4-object-card" data-choice="'+v+'" aria-label="'+label+'" aria-pressed="'+(String(this.setup[key as keyof Setup])===value)+'">'+art+'<span>'+label+'</span></button>';
  ...
  private action(id:string){
    if(/^category-[0-5]$/.test(id)){this.tunePage=Number(id.slice(-1));this.render();this.tell(['齒輪','輪胎','輪徑','車殼','配重','車色'][this.tunePage]);return;}
    if(/^lesson-[0-3]$/.test(id)){this.lesson=Number(id.slice(-1));this.selected=lessons[this.lesson].part;this.xray=true;this.render();this.tell(lessons[this.lesson].text,lessons[this.lesson].voice);return;}
    if(id.startsWith('track-')&&!this.running){const k=id.slice(6) as TrackKind;if(k in tracks){this.stop();this.track=k;this.shadow=false;this.view?.setTrack(k);this.render();this.tell(tracks[k],k==='flat'?'平坦的跑道，先練習三圈。':k==='bridge'?'連續的坡橋，觀察上坡和下坡的速度。':'斜坡後面有缺口，觀察車子起跳和落地。');}return;}
    ...
    else if(id==='ghost'){if(!this.view?.hasGhost()){this.tell('先完成三圈，留下影子！','先在這條跑道完成三圈，就能留下影子。下一次打開影子按鈕，和上一次的自己比賽。影子不會碰撞，只在這次實驗中保留。');return;}this.shadow=!this.shadow;if(!this.shadow)this.view.hideGhost();this.render();this.tell(this.shadow?'影子陪跑，挑戰自己！':'影子休息。','淡紫色的車是上一次完成的自己，不會和你的車碰撞。一次只改一個條件，看看誰先到終點。');}
  ...
  private updateRace(c:CarState){if(this.page!=='race')return;const shadowHud=this.root.querySelector<HTMLElement>('[data-shadow]')!;shadowHud.hidden=this.view?.ghostTime()===undefined;if(!shadowHud.hidden)shadowHud.querySelector('b')!.textContent=this.view!.ghostTime()!.toFixed(2);this.root.querySelector('[data-speed]')!.textContent=(Math.hypot(c.vx,c.vz)*.18).toFixed(1);this.root.querySelector('[data-lap]')!.textContent=`${c.lap} / 3`;this.root.querySelector('[data-time]')!.textContent=c.time.toFixed(1);if(this.running&&(c.finished||c.offTrack)){this.running=false;this.saveRecord(c);const baseline=this.view?.ghostTime();const difference=this.view?.isCollisionRun()?'雙車測試完成':baseline===undefined?'影子已記住！':Math.abs(c.time-baseline)<.02?'和影子同時到！':`${c.time<baseline?'快了':'慢了'} ${Math.abs(c.time-baseline).toFixed(2)} 秒`;this.tell(c.finished?`三圈 ${c.time.toFixed(2)} 秒 · ${difference}`:'車子離開賽道了。回去調整輪胎與配重，再試一次。');this.render();}}
  private saveRecord(c:CarState){if(this.recorded)return;this.recorded=true;this.records.push({setup:{...this.setup},track:this.track,time:c.time,max:c.maxSpeed,contacts:c.contacts,landings:c.landings,finished:c.finished});}
  private answer(id:string){const correct=['battery-a','shaft','rollers'][this.quizIndex];const text=id===correct?'答對了！'+PARTS.find(p=>p.id===correct)!.voice:'再想想。點語音，聽聽題目。';this.root.querySelector('.m4-feedback')!.textContent=id===correct?'✓ 答對了':'↻ 再試試';this.voice.say(text);}
