...
export class CubeTeacher{
 ...
 private showReason(){const c=this.current;if(!c)return;const move=c.moves[this.step]||c.moves[c.moves.length-1]||'';const face=move?CUBE_FACES.find(f=>f.id===move[0]):undefined;const note=approachNotes[c.phase];
  ...
  <p>${note.beginner[1]}</p>
  ${move&&face?`<p><b>下一步 ${move}</b>：先以 ${names[face.color]}色中心當座標。這一步的目的不是只把一張貼紙轉漂亮，而是讓目標零件更接近「自己的中心組合」，或先騰出不破壞已完成區域的位置。</p>`:''}
 ...
 private render(){const c=this.current;if(!c)return;
  ...
  if(this.screen==='colors'){
   ...
   this.panel.innerHTML=`${this.header()}<p class="ct-prompt">找這個中心 ${swatch(face.color)}</p><div class="ct-face-choices">${CUBE_FACES.map(f=>control(`face-${f.id}`,`教學：選${f.name}`,swatch(f.color))).join('')}</div><div class="ct-move-picture" aria-label="${face.name}，${arrow==='↶'?'逆':'順'}時針四分之一圈">${swatch(face.color)}<b>${arrow}</b><small>¼ 圈</small></div><div class="ct-direction">${control('left','教學：逆時針轉一步','↶')}${control('right','教學：順時針轉一步','↷')}</div><p class="ct-feedback" role="status">${this.selected?'正看這一面，跟箭頭轉':'先找中心，鏡頭會對正'}</p>${control('reason','看判斷原因','為什麼是這一步？')}<small>動作 ${this.step+1}/${c.moves.length} · 小段完成會檢查</small>`;
