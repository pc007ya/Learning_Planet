...
import {CubeBook,BOOK_ICON} from './cube-book';
...
export class CubeLab {
  ...
  constructor(private host:HTMLElement){
    host.innerHTML=`<section class="cube-lab" aria-label="魔術方塊工坊">
      ...
      <aside class="cube-tools"><div class="cube-play-tools"><div class="cube-modes">${button('view','⤢<small>轉視角</small>','轉視角模式')}${button('turn','↶<small>轉一層</small>','转一層模式')}${button('inspect','◇<small>拆解</small>','拆解開關')}</div><div class="cube-face-grid">${CUBE_FACES.map(f=>`<button data-face="${f.id}" aria-label="選${f.name}" title="${f.name}" style="--face:#${CUBE_COLORS[f.color].toString(16).padStart(6,'0')}"><i></i><small>${f.id}</small></button>`).join('')}</div><div class="cube-arrows">${button('ccw','↶','選取面逆時針轉四分之一圈')}${button('cw','↷','選取面順時針轉四分之一圈')}</div><div class="cube-face-grid cube-slice-grid" aria-label="中層轉動">${button('slice-M','M','M 中間直層，方向同 L')}${button('slice-Mp','M′','M 中間直層反向')}${button('slice-E','E','E 中間橫層，方向同 D')}${button('slice-Ep','E′','E 中間橫層反向')}${button('slice-S','S','S 前後中層，方向同 F')}${button('slice-Sp','S′','S 前後中層反向')}</div><div class="cube-part-grid" hidden>${parts.map((p,i)=>button(p[0],`${i+1}<small>${p[1]}</small>`,p[1])).join('')}${button('all','◈<small>全部</small>','顯示全部零件')}</div><p class="cube-explanation">選一個面，再轉轉看</p><div class="cube-bottom-tools">${button('shuffle','⤨','打亂')}${button('undo','↩','退一步')}${button('reset','⟲','重新開始')}${button('demo','▶','分鏡導覽')}</div></div><div class="cube-help-tools" hidden><div class="cube-help-tabs">${button('mechanism','◇','機構')}${button('formula','R′','公式')}${button('hint','💡','還原提示')}${button('assets','▧','影片素材')}</div><div class="cube-help-content"></div></div><div class="cube-shot-nav" hidden>${button('previous','‹','上一鏡')}<span data-shot></span>${button('next','›','下一鏡')}${button('stop-demo','■','結束導覽')}</div></aside></div>
    </section>`;
  ...
  private update(){this.view.select(this.face);this.root.querySelectorAll<HTMLButtonElement>('[data-face]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.face===this.face)));for(const m of ['view','turn','inspect'])this.root.querySelector(`[data-cube=${m}]`)!.setAttribute('aria-pressed',String(m===this.mode));
    this.root.querySelector<HTMLElement>('.cube-part-grid')!.hidden=this.mode!=='inspect';this.root.querySelectorAll<HTMLElement>('.cube-face-grid').forEach(el=>el.hidden=this.mode==='inspect');this.root.querySelector<HTMLElement>('.cube-arrows')!.hidden=this.mode==='inspect';
  ...
  private async action(id:string){
    ...
    else if(/^slice-(M|E|S)p?$/.test(id)){await this.setMode('turn');const code=id.slice(6,7)+(id.endsWith('p')?"'":"");await this.turn(code);this.tell(code[0]==='M'?'M：左右之間的中層，方向以 L 面為參考。':code[0]==='E'?'E：上下之間的中層，方向以 D 面為參考。':'S：前後之間的中層，方向以 F 面為參考。',`${code} 中層轉動`);}
    ...
    else if(id==='shuffle'){await this.setMode('turn');const token=++this.sequence;const pool=[...CUBE_FACES.map(f=>f.id),'M','E','S'];for(let i=0;i<10&&!this.dead&&token===this.sequence;i++){const base=pool[Math.floor(Math.random()*pool.length)];await this.turn(base+(Math.random()<.5?"'":""));}this.tell('換你挑戰！這次也可能有 M、E、S 中層轉動。點教我還原，先觀察中心座標，再一步一步判斷。','打亂完成 · 含中層挑戰');}
  ...
  private help(tab:string){const box=this.root.querySelector('.cube-help-content')!;this.root.querySelectorAll('[data-cube=mechanism],[data-cube=formula],[data-cube=hint],[data-cube=assets]').forEach(b=>b.setAttribute('aria-pressed',String((b as HTMLElement).dataset.cube===tab)));
    ...
    if(tab==='formula'){box.innerHTML=`<h3>跟著轉一次</h3><div class="cube-formula">R → U<br>R′ → U′</div>${button('formula-play','▶','播放四步公式')}<p>外層：R L U D F B<br>中層：M E S</p><p>M 同 L 方向 · E 同 D 方向 · S 同 F 方向<br>′ 反轉 · 2 半圈</p><small>公式只是語言；真正要先看零件與中心的關係。</small>`;this.tell('除了六個外層，現在也能直接轉 M、E、S 中層。M 方向跟 L 一樣，E 跟 D 一樣，S 跟 F 一樣。');}
  ...
  destroy(){this.dead=true;this.sequence++;this.book.destroy();this.teacher?.destroy();this.recorder?.destroy();this.voice.stop();this.abort.abort();this.view.destroy();if(this.previewURL)URL.revokeObjectURL(this.previewURL);this.host.replaceChildren();}
}
