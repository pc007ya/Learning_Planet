...
import {makeChassis} from './mini4wd-chassis';
...
export class MiniCarModel{
  ...
  private street=new T.Group();private racer=new T.Group();
  ...
  constructor(){
    ...
    const ch=this.parts.get('chassis')!;ch.add(makeChassis());
    ...
    const body=makeSportsShell();shell.add(body.root);this.wings=body.wing;this.street=body.street;this.racer=body.racer;this.skins=[body.paint];this.shellMaterials=body.materials;
  ...
  configure(s:Setup){this.skins.forEach(m=>m.color.set(s.color));this.rubber.forEach(m=>{m.color.set(s.tire==='grip'?0x213346:0x687585);m.roughness=s.tire==='grip'?.92:.45;});this.wings.visible=s.shell==='wing';this.street.visible=s.shell==='arrow';this.racer.visible=s.shell==='wing';this.ballast.position.x=s.ballast==='rear'?-.67:0;for(const w of this.wheelParts)w.scale.setScalar(s.diameter/26);}
  layout(installed:Set<string>,explode:number,xray:boolean,_selected:string){
    ...
    for(const m of this.shellMaterials){const alpha=xray?.12:(m.userData.normalOpacity??1),transparent=alpha<1;if(m.transparent!==transparent)m.needsUpdate=true;m.transparent=transparent;m.opacity=alpha;m.depthWrite=!transparent;}
  }
