...
修正了拆解與核心近看鏡頭過近造成的裁切。透明 PNG 已檢視並確認 alpha 通道。WebM 已由實際 UI 匯出並核對完整資料：960×640、約 25.57 秒、3,129,715 bytes、無聲 VP8。

## 正式站驗收完成

- 功能版本：`d0d59e39c9bf8acbf09d941b3f3b90e4d114ce26`。
- GitHub Pages 部署成功：https://github.com/pc007ya/Learning_Planet/actions/runs/34091811080 。雲端完整測試與兩套模組建置通過。
- 正式站直接視窗量測（非 iframe）：1280×720 完整方塊、1024×600 拆解／核心／素材面板、1024×768 看圖題。頁面寬高均等於視窗寬高，沒有畫面外控制項。
- 實際操作：手動轉視角、放大／縮小、看圖選邊塊得到正確回饋、四步公式播放、依紀錄倒轉四步後恢復六面、语音開關與重聽、返回實驗清單。
- 既有回力車可正常載入；本輪正式站頁面未記錄到 console error。未宣稱實體 iPad 或所有語音引擎實測。
- 正式站影片 SHA-256 與本機一致：`febadb12fdc3acdf82bc182e057590ec6d61a97e3ae40ccf3f44422bada78ebe`。
- 截圖保存在工作區 `驗收截圖/2026-09-07-方塊工坊/`：正式站-1280x720-完整方塊、正式站-1024x600-拆解、正式站-1024x600-核心、正式站-1024x768-看圖題。
- 窄版直向不是本輪橫放驗收範圍，仍可能出現必要捲動。未把小尺寸模擬說成實體裝置驗收。
