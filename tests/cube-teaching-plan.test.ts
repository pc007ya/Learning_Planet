...
describe('state-based cube teacher',()=>{
 ...
 for(let seed=1;seed<=300;seed++)it(`solves independently of history, seed ${seed}`,()=>{
  ...
  for(let i=0;i<plan.length;i++){const chapter=plan[i];state=applyCubeMoves(state,chapter.moves);expect(faceletString(state)).toBe(chapter.end);expect(state).toEqual(chapter.checkedState);
