...
describe('compact clock workspace', () => {
  it('puts icon switches in the heading and exposes the hidden coaxial pinion',()=>{
    const code=readFileSync('src/experiments/mechanism.ts','utf8');expect(code).toContain("host.querySelector('.bp-heading')!.append(tools.firstElementChild!);tools.remove()");expect(code).toContain("this.kind==='clock'&&teeth>=36");expect(code).toContain('36T＋10T · 同軸');
  });
