...
describe('exact classic 3×3 cube moves',()=>{
  ...
  for(const face of ['R','L','U','D','F','B','M','E','S'])it(`${face} has exact inverse and four-quarter-turn identity`,()=>{
  ...
  it('uses Singmaster slice directions: M like L, E like D, S like F',()=>{
    const m=turnCube(solvedCube(),'M').find(p=>p.id==='0,1,1')!;
  ...
  it('rejects unsupported notation instead of silently corrupting a state',()=>{
    for(const move of ['x','Rw','R3','F<script>',''])expect(()=>turnCube(solvedCube(),move)).toThrow();
  });
});
