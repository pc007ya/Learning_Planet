...
describe('Gail and the Moonlight Garden',()=>{
 it('ships nonempty environment atlases and a separate Gail on every page',()=>{
  for(const scene of Object.values(moonlightScenes))expect(statSync('images/story/moonlight-garden-v2/'+scene.sheet).size).toBeGreaterThan(1000);
  for(const page of moonlightPages('B'))expect(page.objects).toContain('gail');
  for(const name of ['gail','gail-seated','frog','moth','bell','foreground'])expect(statSync('images/story/moonlight-garden-v2/'+name+'.png').size).toBeGreaterThan(1000);
 });
