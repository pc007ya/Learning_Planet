import { h as R, B as H, j as T, q as V, r as Z, d as b, I as _, O as X, s as U, t as E, u as $, k as C, n as y, v as D, V as g, w as N, x as K, y as Q, F as J, z as B, m as G, A as tt, J as et, p as O, a as W, G as x, l as w, K as st, N as at, U as it, X as I, Y as ot, Z as rt, D as nt, W as ct, _ as ht, $ as lt, a0 as dt, P as ut } from "./RoundedBoxGeometry-CV-2QMmK.mjs";
import { L, F as A, i as pt, b as j, c as mt } from "./car-art-D34K7c64.mjs";
...
function gt(m) {
  ...
  }, r = (n, d, c, p, h, l, u = 0) => s(new G(n, d, c, 3, Math.min(n, d, c) * 0.28), e(p), h, l, u), a = new E();
  a.moveTo(-1.96, -0.22), a.lineTo(-1.754, -0.22), a.absarc(-1.18, -0.3, 0.58, Math.PI - 0.14, 0.14, !0), a.lineTo(0.606, -0.22), a.absarc(1.18, -0.3, 0.58, Math.PI - 0.14, 0.14, !0), a.lineTo(1.96, -0.22), a.bezierCurveTo(2.06, -0.02, 2.02, 0.34, 1.84, 0.48), a.bezierCurveTo(1.39, 0.6, 0.87, 0.57, 0.54, 0.58), a.lineTo(-1.55, 0.58), a.bezierCurveTo(-1.91, 0.56, -2.03, 0.22, -1.96, -0.22), a.closePath(), s(new $(a, { depth: 1.5, bevelEnabled: !0, bevelSize: 0.045, bevelThickness: 0.045, bevelSegments: 3, curveSegments: 32 }), t(), 0, 0, -0.75);
  const i = new E();
  i.moveTo(-1.43, 0.52), i.lineTo(-1.02, 1.31), i.quadraticCurveTo(-0.96, 1.38, -0.8, 1.38), i.lineTo(0.23, 1.38), i.quadraticCurveTo(0.33, 1.38, 0.4, 1.27), i.lineTo(0.88, 0.55), i.closePath(), s(new $(i, { depth: 1.24, bevelEnabled: !0, bevelSize: 0.045, bevelThickness: 0.04, bevelSegments: 3 }), t(), 0, 0, -0.62);
  const o = (n, d = 1255995) => {
    const c = new Q();
  ...
  for (const n of [-1, 1]) {
    const d = 0.667 * n;
    o([[-1.29, 0.65, d], [-0.98, 1.27, d], [-0.54, 1.27, d], [-0.54, 0.65, d]]), o([[-0.47, 0.65, d], [-0.47, 1.27, d], [0.26, 1.27, d], [0.72, 0.65, d]]), r(0.055, 0.66, 0.038, 1582382, -0.5, 0.97, d), r(3.16, 0.026, 0.04, 12179163, -0.03, 0.55, 0.801 * n), r(0.27, 0.06, 0.06, 14870505, -0.12, 0.43, 0.815 * n), r(1.08, 0.085, 0.045, 2108725, 0, -0.22, 0.815 * n), s(new C(0.13, 20, 12), e(15855333), 0.67, 0.68, 0.89 * n).scale.set(1.15, 0.8, 0.66);
    for (const h of [-1.18, 1.18]) s(new y(0.588, 0.045, 8, 36, Math.PI), e(2437690), h, -0.3, 0.805 * n);
  ...
  for (const n of [-0.5, 0.5]) {
    ...
    d.scale.x = 0.27;
    const c = s(new y(0.185, 0.024, 8, 32), e(14410211, 0.8), 2, 0.25, n);
...
function z(m) {
  const t = m * 2 * Math.PI, e = -t * 12 / 36, s = -e * 12 / 48, r = -s * 12 / 60;
  return { motor: t, first: e, second: s, minute: r };
}
const xt = [
...
class kt {
  constructor(t, e, s, r, a, i, o, n) {
    ...
    }, { signal: this.abort.signal }), t === "clock" ? this.clock() : this.car(), e.add(this.root), this.skins(), r.addEventListener("pointerdown", (h) => {
      h.button || h.target.closest("button,details") || t === "car" && this.carMode === "idle" || (this.dragYaw = this.yaw, this.dragPull = this.pullValue, this.dragging = { x: h.clientX, minutes: this.minutes, moved: !1 }, r.setPointerCapture(h.pointerId));
    }, { signal: this.abort.signal }), r.addEventListener("pointermove", (h) => {
    ...
    }, { signal: this.abort.signal }), r.addEventListener("pointerup", (h) => {
      if (!this.dragging) return;
      const l = this.dragging.moved;
      if (this.dragging = null, r.hasPointerCapture(h.pointerId) && r.releasePointerCapture(h.pointerId), l && this.mode === "whole" && (t === "clock" && !this.clockRotate || this.carMode === "test")) this.action(-1, !0);
      else if (!l) {
        const u = r.getBoundingClientRect(), f = new O();
        f.setFromCamera(new W((h.clientX - u.left) / u.width * 2 - 1, 1 - (h.clientY - u.top) / u.height * 2), s);
        const v = f.intersectObject(this.root, !0);
        for (const q of v) {
          let M = q.object;
  ...
  mesh(t, e, s, r = 0, a = 0, i = 0, o = 0.1) {
    const n = new b(t, this.material(e, o));
    return n.position.set(r, a, i), s.add(n), n;
  }
  box(t, e, s, r, a, i = 0, o = 0, n = 0) {
    return this.mesh(new G(t, e, s, 3, Math.min(t, e, s) * 0.16), r, a, i, o, n);
  }
  ...
  part(t, e, s, r = !1) {
    ...
    return this.labelLayer.querySelector("svg").append(o), this.parts.push({ id: t, group: a, home: new g(...e), away: new g(...this.kind === "clock" ? yt[t] : s), skin: r, button: i, line: o }), a;
  }
  gear(t, e, s, r, a, i, o = 14265434) {
    ...
    const d = new E();
    for (let h = 0; h < e * 4; h++) {
      const l = h * Math.PI * 2 / (e * 4), u = t * (h % 4 === 0 || h % 4 === 3 ? 0.9 : 1.05), f = Math.cos(l) * u, v = Math.sin(l) * u;
      h ? d.lineTo(f, v) : d.moveTo(f, v);
    }
    d.closePath();
    const c = this.kind === "clock" && e >= 36, p = new st();
    if (p.absarc(0, 0, t * (c ? 0.68 : 0.16), 0, Math.PI * 2, !0), d.holes.push(p), this.mesh(new $(d, { depth: this.kind === "clock" ? 0.05 : 0.12, bevelEnabled: !0, bevelSize: 8e-3, bevelThickness: 8e-3, bevelSegments: 2 }), o, n, 0, 0, 0, 0.65), c) for (let h = 0; h < 3; h++) {
      const l = h * Math.PI * 2 / 3, u = this.box(0.035, t * 0.62, 0.08, o, n, Math.sin(l) * t * 0.43, Math.cos(l) * t * 0.43, 0.06);
      u.rotation.z = -l;
    }
  ...
  clock() {
    ...
    const o = new at(a);
    o.colorSpace = it, this.textures.push(o), this.mesh(new I(1.6, 96), 16314847, r, 0, 0, -6e-3);
    const n = this.mesh(new I(1.6, 96), 16777215, r);
    n.name = "clock-marks", n.material.map = o, n.material.roughness = 0.7, n.material.side = B;
    ...
    for (let u = 0; u < 16; u++) this.mesh(new y(0.12, 0.01, 6, 20), 15049082, p, -0.23 + u * 0.03, 0, 0, 0.8).rotation.y = Math.PI / 2;
    ...
    for (const [u, f, v, q] of [[-1.08, -0.48, -0.31, 0.28], [-0.72, 0, -0.09, 0.28], [0, 0, 0.1, 0.26]]) this.disk(0.025, q, 13490664, c, u, f, v);
  ...
  car() {
    ...
    for (const c of [-1.18, 1.18]) for (const p of [-0.88, 0.88]) {
      const h = new x();
      h.position.set(c, 0.06, p), h.userData.side = p < 0 ? -1 : 1, s.add(h), this.mesh(new y(0.36, 0.145, 16, 48), 2106923, h).scale.z = 1.2, this.disk(0.28, 0.19, 12174797, h), this.disk(0.235, 0.2, 2438206, h), this.disk(0.09, 0.25, 13359326, h);
      const l = p > 0 ? 0.13 : -0.13;
      this.mesh(new y(0.27, 0.022, 8, 40), 14870763, h, 0, 0, l, 0.8);
      for (let u = 0; u < 5; u++) {
        const f = u * Math.PI * 2 / 5, v = this.box(0.048, 0.17, 0.035, 14542313, h, Math.sin(f) * 0.165, Math.cos(f) * 0.165, l);
        v.rotation.z = -f;
      }
      this.wheels.push(h);
    }
    const r = this.part("axles", [0, 0.07, 0], [0, -0.58, 0]);
    for (const c of [-1.18, 1.18]) this.disk(0.07, 1.83, 13488855, r, c, 0, 0);
    const a = this.part("housing", [-0.65, 0.5, 0], [-0.95, 0.7, -1.7], !0);
    this.box(1.25, 0.65, 0.8, 14801605, a);
    const i = this.part("spring", [-0.58, 0.52, 0.1], [-1.8, 0.6, 1.1]), o = Array.from({ length: 300 }, (c, p) => {
      const h = p * 0.18, l = 0.035 + p * 1e-3;
      return new g(Math.cos(h) * l, Math.sin(h) * l, 0);
    });
    this.spring = this.mesh(new N(new D(o), 300, 0.022, 6, !1), 12108750, i, 0, 0, 0, 0.8);
  ...
  select(t, e) {
    ...
    const s = P[this.kind].find((r) => r[0] === t);
  ...
  setClock(t) {
    ...
    const r = [...e, s.motor, s.first, s.first, s.second, s.second, s.minute];
    this.gears.forEach((a, i) => a.rotation.z = r[i]), this.rotor.rotation.z = s.motor;
  }
  setClockDemo(t) {
  ...
  frame(t) {
    ...
    if (e && s) {
      const a = this.kind === "clock" && this.stage.querySelector(".mech-bom[open]") ? 260 : 0, i = this.kind === "clock" ? this.stage.querySelector(".clock-transmission[open]") : null, o = i ? i.offsetWidth + 20 : 0, n = this.kind === "clock" ? 2.6 + this.amount * 3.3 : this.carMode === "test" ? 5.8 : 3.3 + this.amount * 1.4, d = Math.max(this.kind === "clock" ? 2.45 + this.amount * 0.9 : 2.35 + this.amount * 0.95, n * s / Math.max(280, e - a - o)) / (this.zoom || 1), c = d * e / s, p = this.kind === "car" ? this.carMode === "test" ? -0.445 + 0.28 * d : 0.45 : 0, h = c * (a - o) / e;
    ...
    if (this.parts.forEach((a) => {
      ...
      const o = this.gears[[4, 5, 7, 9][i]].getWorldPosition(new g()).project(this.camera), n = [[-25, 18], [-55, -22], [-55, -35], [15, -48]];
      a.style.left = `${(o.x + 1) * e / 2 + n[i][0]}px`, a.style.top = `${(1 - o.y) * s / 2 + n[i][1]}px`;
    }), this.compoundTag) {
    ...
    if (this.halo) {
      ...
      if (this.halo.hidden = !a || this.mode === "whole" && !["case", "glass", "dial", "hands"].includes(this.selected), a) {
        const i = new ot().setFromObject(a.group), o = [];
        ...
        const n = Math.min(...o.map((c) => c.x)) - 8, d = Math.min(...o.map((c) => c.y)) - 8;
        Object.assign(this.halo.style, { left: `${n}px`, top: `${d}px`, width: `${Math.max(...o.map((c) => c.x)) - n + 8}px`, height: `${Math.max(...o.map((c) => c.y)) - d + 8}px` });
      }
    }
    const r = [];
    return this.parts.forEach((a) => {
      const i = a.group.getWorldPosition(new g()).project(this.camera), o = { x: (i.x + 1) * e / 2, y: (1 - i.y) * s / 2 }, d = [[0, 0], [40, 0], [-40, 0], [0, 40], [0, -40], [40, 40], [-40, -40], [60, -40], [-60, 40], [0, 80]].map(([c, p]) => ({ x: Math.max(20, Math.min(e - 20, o.x + c)), y: Math.max(20, Math.min(s - 20, o.y + p)) })).find((c) => r.every((p) => Math.hypot(p.x - c.x, p.y - c.y) > 37)) || o;
      a.button.hidden = this.mode !== "explode" && a.id !== this.selected, a.button.hidden || r.push(d), a.button.style.left = `${d.x}px`, a.button.style.top = `${d.y}px`, a.button.classList.toggle("selected", a.id === this.selected), a.line.setAttribute("x1", String(o.x)), a.line.setAttribute("y1", String(o.y)), a.line.setAttribute("x2", String(d.x)), a.line.setAttribute("y2", String(d.y)), a.line.style.display = a.button.hidden ? "none" : "";
...
const Y = (m) => m >= 0.999999 ? String(Math.round(m)) : `1/${Math.round(1 / m)}`;
function Mt(m) {
  return Object.values(wt(m)).map(Y);
}
function St(m) {
  return Object.values(z(m)).map((t) => Y(Math.abs(t) / (2 * Math.PI)));
}
const Ct = (m, t) => `<svg viewBox="0 0 48 48" aria-hidden="true"><circle cx="24" cy="24" r="17" fill="${m}" stroke="${m}" stroke-width="8" stroke-dasharray="5 4"/><circle cx="24" cy="24" r="7" fill="#102740"/><text x="24" y="28" text-anchor="middle" fill="white" font-size="11">${t}</text></svg>`, k = (m, t, e, s, r) => `<div class="ct-row">${Ct(m, t)}<div>${e}<strong ${r}>${s} RPM</strong></div></div>`;
function Tt() {
  return `<details class="clock-transmission" open><summary aria-label="收折或展開轉速圖解">⚙ 轉速圖解 <span>⌃</span></summary><div class="ct-body"><div class="ct-tools"><button data-rpm-speed="1" aria-pressed="true">正常</button><button data-rpm-speed="60" aria-pressed="false">60×</button><button data-rpm-size aria-label="放大轉速面板" aria-pressed="false">⤢</button><button data-rpm-read aria-label="朗讀轉速圖解">🔊</button></div><div class="ct-tabs"><button data-drive-page="motor" aria-pressed="true">馬達</button><button data-drive-page="hands" aria-pressed="false">指針</button></div><div data-drive-section="motor">${k("#77efbd", "12", "↺ 馬達 · 12T", "1", "data-drive-rpm")}<div class="ct-link">↓ 慢 3 倍</div>${k("#77efbd", "36", "↻ 36T／同軸12T", "1/3", "data-drive-rpm")}<div class="ct-link">↓ 再慢 4 倍</div>${k("#77efbd", "48", "↺ 48T／同軸12T", "1/12", "data-drive-rpm")}<div class="ct-link">↓ 再慢 5 倍</div>${k("#77efbd", "60", "↻ 60T → 長分針", "1/60", "data-drive-rpm")}</div><div data-drive-section="hands" hidden>${k("#52c9d7", "12", "↻ 長分針 · 12T", "1/60", "data-rpm")}<div class="ct-link">↓ 慢 3 倍 · 反方向</div>${k("#dcbd69", "36", "↺ 36T／同軸10T", "1/180", "data-rpm")}<div class="ct-link">↓ 再慢 4 倍 · 反方向</div>${k("#e2a954", "40", "↻ 短時針 · 40T", "1/720", "data-rpm")}</div><footer>馬達 60 圈＝長針 1 圈<small>長針 12 圈＝短針 1 圈<br>RPM＝每分鐘轉幾圈。完整教學輪系，非品牌機芯；正常／60× 僅切換數值對照。</small><span class="ct-progress">60× · <span data-demo-seconds>0</span> / 60 秒</span></footer></div></details>`;
}
class $t {
  constructor(t, e) {
    ...
    if (r.className = "il-scene-guide", r.textContent = e === "buoyancy" ? "選物品 → 預測 → 放入水中 → 比較浮沉" : e === "clock" ? "左：指針比例 12:1 ｜ 右：外齒輪反向傳動示意" : "向左回拉 → 彈簧儲能 → 放手帶動輪軸", this.stage.append(r), t.querySelector(".il-controls").innerHTML = e === "buoyancy" ? `<label>① 選物品<select data-input="object">${A.map((i, o) => `<option value="${o}">${i.name}</option>`).join("")}</select></label><p data-sample></p><label>② 先預測<select data-input="prediction"><option value="">請選擇</option><option value="float">浮在水面</option><option value="sink">沉到水底</option><option value="unsure">還不確定</option></select></label><button type="button" data-action="run">③ 放入水中</button><p>也可拖動物品，放手入水。可重複測試，猜錯也能學習。</p>` : e === "clock" ? '<label>轉動分針（分鐘）<input data-input="minutes" type="range" min="0" max="720" step="1" value="0"></label><p data-readout>12:00</p><button type="button" data-action="step">分針前進一圈（60 分鐘）</button><button type="button" data-action="run">▶ 播放／暫停</button><button type="button" data-action="record">記錄目前時間</button><p>可拖曳鐘面指針；滑桿與按鈕也能操作。</p>' : '<label>① 向後拉多少<input data-input="pull" type="range" min="10" max="100" step="10" value="50"></label><p data-readout>回拉量 50%</p><label>② 路面<select data-input="surface"><option value="smooth">較平滑</option><option value="rough">較粗糙</option></select></label><button type="button" data-action="run">③ 放手出發</button><p>也可把車向左拖，放手前進。比較同一回拉量在不同路面的結果。</p>', e === "car") {
      ...
      o.className = "car-track", o.setAttribute("aria-hidden", "true"), this.stage.append(o), pt(t);
    }
    if (e === "clock") {
    ...
    e === "buoyancy" && this.buildTank(), e !== "buoyancy" && (this.mechanism = new kt(e, this.scene, this.camera, this.stage, t, this.renderer, () => this.wake(), (i, o) => {
  ...
  mesh(t, e, s = 0, r = 0, a = 0, i = this.scene) {
    const o = new b(t, new T({ color: e, roughness: 0.55, metalness: 0.12 }));
    return o.position.set(s, r, a), i.add(o), o;
  }
  box(t, e, s, r, a, i, o = 0, n) {
    return this.mesh(new H(t, e, s), r, a, i, o, n);
  }
  buildTank() {
  ...
  disposeObject(t) {
    t.traverse((e) => {
      e instanceof b && (e.geometry.dispose(), (Array.isArray(e.material) ? e.material : [e.material]).forEach((r) => r.dispose()));
    });
  }
  buildSample() {
  ...
  click = (t) => {
    ...
    if (e) {
      ...
      if (e.dataset.rpmSpeed) {
        const s = e.dataset.rpmSpeed === "60" ? 60 : 1;
        this.host.querySelectorAll("[data-rpm-speed]").forEach((i) => i.setAttribute("aria-pressed", String(i === e)));
        const r = Mt(s), a = St(s);
        this.host.querySelectorAll("[data-rpm]").forEach((i, o) => i.textContent = r[o] + " RPM"), this.host.querySelectorAll("[data-drive-rpm]").forEach((i, o) => i.textContent = a[o] + " RPM");
        return;
      }
      ...
      if (this.kind === "clock" && (["quiz", "notes", "help"].includes(e.dataset.helper || "") || e.hasAttribute("data-clock-rotate") || e.hasAttribute("data-clock-explode") || e.dataset.action === "step") && (this.active = !1, this.updateClock()), this.kind === "clock" && (e.hasAttribute("data-clock-rotate") || e.hasAttribute("data-clock-explode") || e.dataset.action === "step") && (this.demoStarted = !1, this.mechanism?.setClockDemo(!1)), this.kind === "car" && ((e.dataset.carMode || e.dataset.part) && (this.active = !1), e.dataset.road && (this.active = !1, this.rough = e.dataset.road === "rough", this.host.dataset.road = e.dataset.road, this.host.querySelectorAll("[data-road]").forEach((s) => s.setAttribute("aria-pressed", String(s === e))), this.updateCar(), this.status.textContent = this.rough ? "換成石頭路，試試看能跑多遠。" : "換成平路，用相同力量比較看看。"), e.dataset.force && (this.active = !1, this.pull = Number(e.dataset.force), this.mechanism?.setCarMode("test"), this.updateCar())), e.dataset.answer !== void 0) {
        if (this.kind === "car") {
          const s = Number(e.dataset.answer) === L.car.answer, r = this.answerReady ? s ? "答對了！往後拉車時，能量存在捲簧裡。放手後，捲簧透過齒輪帶動車輪。" : "再想想看！回到實驗，拆開車殼看看捲簧，聽聽它的工作。" : "先回到實驗，完成一次拉車試跑，再來選圖片。";
  ...
  run() {
    if (this.kind === "buoyancy") {
      ...
      const t = A[this.selected], e = j(t.mass, t.volume);
  ...
  point(t) {
    ...
    const r = new g();
    return s.ray.intersectPlane(new ut(new g(0, 0, 1), 0), r), r;
  }
  pointerDown = (t) => {
  ...
  tick = (t) => {
    ...
    if (this.last = t, this.active && this.kind === "buoyancy" && (this.object.position.y += (this.targetY - this.object.position.y) * Math.min(1, s * 3), Math.abs(this.object.position.y - this.targetY) < 0.015)) {
      this.active = !1;
      const a = A[this.selected], i = j(a.mass, a.volume);
