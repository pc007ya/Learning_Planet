import { J as E, K as A, q as T, T as M, f as H, V as k, e as D, b as y, M as V, B as Y, F as _, D as N, R as B, a as L, g as X, A as Z, j as G, k as O, G as w, C, N as U, U as K, X as Q, Y as I, n as J, Z as tt, h as et, W as st, S as at, _ as it, c as ot, $ as rt, a0 as nt, w as ct } from "./RoundedBoxGeometry-DFERVGq8.mjs";
import { L as q, F as $, i as ht, b as j, c as lt } from "./car-art-D34K7c64.mjs";
import { R as dt } from "./RoomEnvironment-DwYDT8-q.mjs";
...
class kt {
  constructor(t, e, s, o, a, i, n, r) {
    ...
    u.className = "mech-quick-info", u.setAttribute("aria-live", "polite"), u.innerHTML = "<strong>當個小小拆解師</strong><p>選「拆開看看」，再點一個零件。</p>", a.querySelector("aside .il-controls").before(u), t === "car" && (u.innerHTML = "<strong>選個開關玩玩看</strong><p>轉一轉、拆一拆，再試跑！</p>"), t === "clock" && (u.innerHTML = "<strong>☝ 拖動調時</strong><p>⚙ 拆卸 · ☷ 找零件</p>"), a.addEventListener("click", (c) => {
      ...
      if (l) {
        ...
        if ((l.hasAttribute("data-clock-rotate") || l.hasAttribute("data-clock-explode")) && (a.querySelector(".clock-switches > span").textContent = this.clockRotate || this.mode === "explode" ? "☝ 拖動旋轉" : "☝ 拖動調時", u.innerHTML = this.mode === "explode" ? "<strong>☷ 找零件</strong><p>點數字，聽聽它的工作。</p>" : this.clockRotate ? "<strong>☝ 拖動旋轉</strong><p>關閉旋轉，就能調時間。</p>" : "<strong>☝ 拖動調時</strong><p>長分針一圈，短時針一格。</p>"), l.hasAttribute("data-clock-zoom")) {
          const m = Number(l.dataset.clockZoom);
          this.zoom = m ? Math.max(0.75, Math.min(1.5, this.zoom + m * 0.15)) : 1, m || (this.yaw = -0.25), this.wake();
        }
      }
    }, { signal: this.abort.signal }), d.addEventListener("input", (c) => {
      const l = c.target;
      l.hasAttribute("data-mech-turn") ? this.yaw = Number(l.value) * Math.PI / 180 : (this.target = Number(l.value) / 100, this.mode = this.target ? "explode" : "whole", d.querySelectorAll("[data-view]").forEach((m) => m.setAttribute("aria-pressed", String(m.dataset.view === this.mode))), this.skins()), this.wake();
    }, { signal: this.abort.signal }), t === "clock" ? this.clock() : this.car(), e.add(this.root), this.skins(), o.addEventListener("pointerdown", (c) => {
      c.button || c.target.closest("button,details") || t === "car" && this.carMode === "idle" || (this.dragYaw = this.yaw, this.dragPull = this.pullValue, this.dragging = { x: c.clientX, minutes: this.minutes, moved: !1 }, o.setPointerCapture(c.pointerId));
    }, { signal: this.abort.signal }), o.addEventListener("pointermove", (c) => {
    ...
    }, { signal: this.abort.signal }), o.addEventListener("pointerup", (c) => {
      if (!this.dragging) return;
  ...
  clock() {
    ...
    this.disk(1.8, 0.5, 2847620, t), this.mesh(new M(1.7, 0.15, 16, 96), 12758396, t, 0, 0, 0.32, 0.8), this.box(0.3, 0.65, 0.5, 2847620, t, -0.95, -1.7), this.box(0.3, 0.65, 0.5, 2847620, t, 0.95, -1.7);
    const e = this.part("glass", [0, 0, 0.7], [3.3, 0.3, 1.4], !0), s = this.disk(1.62, 0.045, 14087679, e);
    s.material.transparent = !0, s.material.opacity = 0.1, s.material.depthWrite = !1, this.mesh(new M(1.64, 0.035, 12, 96), 10470606, e, 0, 0, 0, 0.7);
    ...
    this.box(0.075, 1.31, 0.04, h(b.minute), this.handMinute, 0, 0.55, 0.07), this.box(0.13, 0.92, 0.05, h(b.hour), this.handHour, 0, 0.34, 0.02), this.disk(0.1, 0.12, 15056229, d, 0, 0, 0.13);
    const u = this.part("gears", [0, 0, 0.08], [0.05, 0.5, 0.1]);
    this.gear(0.2, 12, u, 0, 0, 0.19, h(b.minute)), this.gear(0.6, 36, u, 0.8, 0, 0.19, h(b.compound)), this.gear(0.16, 10, u, 0.8, 0, -0.04, h(b.compound)), this.compoundTag = document.createElement("span"), this.compoundTag.className = "clock-compound-tag", this.compoundTag.textContent = "36T＋10T · 同軸", this.compoundTag.style.borderColor = b.compound, this.stage.append(this.compoundTag), this.gear(0.64, 40, u, 0, 0, -0.04, h(b.hour)), this.disk(0.055, 0.45, 13490664, u, 0.8, 0, 0.1);
    ...
    const m = this.part("battery", [0, -1.25, -0.48], [-2.6, -1.6, -0.1]);
    this.mesh(new C(0.19, 0.19, 1.25, 32), 13351808, m, 0, 0, 0, 0.6).rotation.z = Math.PI / 2, this.mesh(new C(0.2, 0.2, 0.27, 32), 2371136, m, 0.47, 0, 0).rotation.z = Math.PI / 2, this.mesh(new C(0.09, 0.09, 0.05, 16), 14278628, m, -0.65, 0, 0, 0.8).rotation.z = Math.PI / 2;
  }
  car() {
    ...
    for (const h of [-1.18, 1.18]) for (const u of [-0.88, 0.88]) {
      const c = new w();
      c.position.set(h, 0.06, u), c.userData.side = u < 0 ? -1 : 1, s.add(c), this.mesh(new M(0.36, 0.145, 16, 48), 2106923, c).scale.z = 1.2, this.disk(0.28, 0.19, 12174797, c), this.disk(0.235, 0.2, 2438206, c), this.disk(0.09, 0.25, 13359326, c);
      const l = u > 0 ? 0.13 : -0.13;
      this.mesh(new M(0.27, 0.022, 8, 40), 14870763, c, 0, 0, l, 0.8);
      for (let m = 0; m < 5; m++) {
        const p = m * Math.PI * 2 / 5, g = this.box(0.048, 0.17, 0.035, 14542313, c, Math.sin(p) * 0.165, Math.cos(p) * 0.165, l);
        g.rotation.z = -p;
      }
      this.wheels.push(c);
    }
