  ...
  constructor(e, s) {
    ...
    if (a.className = "il-mission-nav", a.setAttribute("aria-label", "實驗小幫手"), a.innerHTML = '<button data-helper="voice" aria-pressed="false">🔊 開始語音陪玩</button><button data-helper="replay">🗣 再講一次</button><button data-helper="notes">📒 發現筆記</button><button data-helper="quiz">🌟 小挑戰</button><button data-helper="help">💡 怎麼玩</button><span class="il-voice-state" role="status">點一下，開啟語音</span>', e.prepend(a), this.voiceButton = a.querySelector('[data-helper="voice"]'), s === "buoyancy" || s === "car" || s === "clock") {
      ...
      if (a.querySelectorAll("button").forEach((n) => {
        const d = n.textContent.replace(/^\S+\s*/, "");
        n.setAttribute("aria-label", d), n.title = d, n.textContent = o[n.dataset.helper];
      }), s === "car" || s === "clock") {
        const n = a.querySelector('[data-helper="quiz"]');
        n.innerHTML = `<img src="${f}pencil.png" alt="" draggable="false">`, n.setAttribute("aria-label", "考題"), n.title = "切換考題";
      }
    }
    ...
    a.addEventListener("click", (i) => {
      ...
      if (t === "voice") {
        ...
        p(o), o ? this.say(c[1].textContent || "") : this.stop();
      } else if (t) {
        (s === "car" || s === "clock") && p(!1);
        const o = ["notes", "quiz", "help"].indexOf(t);
        if (o < 0) return;
        c.forEach((n, d) => n.toggleAttribute("hidden", d !== o)), this.dialog.showModal(), this.say(c[o].textContent || "");
      }
      ...
      for (const o of t) {
        ...
        if (n && i.some((d) => n === d.target || n.contains(d.target))) {
          if (o === ".il-live" && n.textContent?.startsWith("準備好了")) continue;
          this.say(o === ".il-log" ? n.firstElementChild?.textContent || "" : n.textContent || "");
          break;
        }
      }
    });
