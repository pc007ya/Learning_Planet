...
document.addEventListener('click',e=>{
  ...
  const frame=document.createElement('iframe');const lesson=opener?.dataset.mathTeachingOpen;const url=new URL(lesson?.startsWith('frac-')?'../../math-fractions.html':lesson==='mul'?'../../math-multiplication.html':'../../math-teaching.html',import.meta.url);if(['ten','add','sub'].includes(lesson))url.searchParams.set('lesson',lesson);if(opener?.dataset.mathTeachingMax==='50')url.searchParams.set('max','50');if(lesson?.startsWith('frac-')){url.searchParams.set('op',lesson==='frac-sub'?'sub':'add');url.searchParams.set('same','1');}frame.src=url.href;frame.title='數學互動教學桌';frame.style.cssText='width:100%;height:100%;border:0;display:block';dialog.append(frame);
